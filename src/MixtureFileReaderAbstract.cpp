...
#if !defined(ALIZE_MixtureFileReaderAbstract_cpp)
#define ALIZE_MixtureFileReaderAbstract_cpp
...
bool R::getBigEndian(const Config& c, BigEndian b) const // protected
{
  ...
  if (b == BIGENDIAN_FALSE)
    return false;
  // BIGENDIAN_AUTO
  if (c.existsParam_loadMixtureFileBigEndian)
    return c.getParam_loadMixtureFileBigEndian();
...
{
  throw Exception("Do not know how to load a mixture GD", __FILE__, __LINE__);
  return MixtureGD::create(K::k, "", 1); // never called
}
//-------------------------------------------------------------------------
const MixtureGF& R::readMixtureGF()
{
  throw Exception("Do not know how to load a mixture GF", __FILE__, __LINE__);
  return MixtureGF::create(K::k, "", 1); // never called
}
//-------------------------------------------------------------------------
R::~MixtureFileReaderAbstract()
{
  if (_pMixture != NULL)
    delete (_pMixture);
  if (_pReader != NULL)
...
#endif // !defined(ALIZE_MixtureFileReaderAbstract_cpp)
