...
real_t S::logsumexp(real_t nums[], size_t ct) {
	real_t max_exp = nums[0], sum = 0.0;
	size_t i;
...
//    {
//      //if (_occVect[c] != 0.0)
//	//!TODO : AQUI HAY UN ERROR , DEBERIA SER DE OTRA MANERA _occVect[c] = 1/_distribCount ?
//		for (c=0; c<_distribCount; c++)
//		{
//			Distrib* d = distribVect[c];
//			occVect[c] = log(weightVect[c]) + d->computeLLK(f);
//			
//		}
//		sum = logsumexp(occVect, (size_t) _distribCount);
//        for (c=0; c<_distribCount; c++)
//		{
//			double Zllk =  (occVect[c] - sum);
//			if ( Zllk < -460)// log(1e-200)
//			{
//				occVect[c] = 0.0;
//			}else{
//				occVect[c] = exp(occVect[c] - sum);
//			}
//			
//		}
//    }
...
real_t S::computeOccVect(const Feature& f) // private
{
	...
	if (sum > LEPS_APP) /* si la trame a un poids non negligeable */
	{
		sum = exp(sum);
	}
	else /* si la trame a un poids negligeable */
