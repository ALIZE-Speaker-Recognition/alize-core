...
#if !defined(ALIZE_TestMixtureGF_cpp)
#define ALIZE_TestMixtureGF_cpp
...
//-------------------------------------------------------------------------
TestMixtureGF::TestMixtureGF()
:Test() {}
//-------------------------------------------------------------------------
bool TestMixtureGF::exec()
{
    Test::exec();
    {
        unsigned long i;
        ...
        test(m1.getClassName() == "MixtureGF", __LINE__);
        ...
        DistribGF& d0 = DistribGF::create(K::k, m1.getVectSize()+1);
        ...
        DistribGF& d1 = DistribGF::create(K::k, m1.getVectSize());
        DistribGF& d2 = DistribGF::create(K::k, m1.getVectSize());
        ...
        //                bool operator==(const MixtureGF&) const
        ...
        {
            {
                MixtureGF m1("", 5, 3);
                MixtureGF m2("", 5, 3);
                for (unsigned long h=0; h<m1.getDistribCount(); h++)
                {
                  DistribGF& d = m1.getDistrib(h);
                  d.getCovMatrix().setAllValues(0.0);
                  d.getCovInvMatrix().setAllValues(0.0);
                }
                for (i=0; i<3; i++)
                ...
                real_t covInv = m2.getDistrib(0).getCovInv(0, 0);
                m2.getDistrib(0)._covInvMatr(0, 0) = 0.123456;
                test(m1 != m2 , __LINE__);
                test(!(m1 == m2) , __LINE__);
                m2.getDistrib(0)._covInvMatr(0, 0) = covInv;
        ...
        // test duplicate
        unsigned long* ref = new unsigned long[m1.getDistribCount()];
        for (i=0; i<m1.getDistribCount(); i++)
            ref[i] = m1.getDistrib(i)._refCounter;
        MixtureGF& m2 = static_cast<MixtureGF&>(m1).duplicate(K::k, SHARE_DISTRIB);
        ...
        MixtureGF& m3 = static_cast<MixtureGF&>(m1).duplicate(K::k, DUPL_DISTRIB);
        ...
        try
        {
            MixtureGF tt("oijiji", m3.getVectSize()+1, m3.getDistribCount());
        ...
        try
        {
            MixtureGF tt("oijiji", m3.getVectSize(), m3.getDistribCount()+1);
            tt = m3;  // distribCount are different
            showError(__LINE__);
        }
        catch (Exception&) {}
        MixtureGF tt("oijiji", m3.getVectSize(), m3.getDistribCount());
        tt = m3;
        for (i=0; i<m3.getDistribCount(); i++)
        {
...
//-------------------------------------------------------------------------
String TestMixtureGF::getClassName() const
{
    return "TestMixtureGF";
}
//-------------------------------------------------------------------------
TestMixtureGF::~TestMixtureGF() {}
//-------------------------------------------------------------------------

#endif // !defined(ALIZE_TestMixtureGF_cpp)
